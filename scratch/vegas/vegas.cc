...
using namespace ns3;
// 本程式碼修改自範例tcp-vegas，大部分維持原狀，改變實驗變數與輸出位址

// 輸出全域變數
std::string dir = "output/vegas/";
...
// TCP Vegas 專用參數變數 2 4 1
uint32_t vegasAlpha = 8;
uint32_t vegasBeta = 16;
uint32_t vegasGamma = 4;
...
>>>>>>> d64e16160de061a9ffe3af750135e6dc116688c6
...
void InstallBulkSend(Ptr<Node> node, Ipv4Address address, uint16_t port,
  ...
  Simulator::Schedule(Seconds(0.1) + Seconds(0.1), &TraceCwnd, nodeId,
...
<<<<<<< HEAD
  ...
  std::string tcpTypeId = "ns3::TcpVegas";
...
<<<<<<< HEAD
  ...
  // 為router安裝queue discipline, queue size = 100 packets
  ...
  udpSinkApps.Start(Seconds(15.0));
  ...
  udpSource.SetAttribute("PacketSize", UintegerValue(1 << 10));
